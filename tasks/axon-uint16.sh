...
then
    ...
    ssh login1 "source /etc/profile; export LAZYFLOW_THREADS=${LAZYFLOW_THREADS}; export LAZYFLOW_TOTAL_RAM_MB=${LAZYFLOW_TOTAL_RAM_MB}; qsub -sync y -pe batch 4 -N ml-${tile_name} -j y -o ${log_file_cluster_1} -b y -cwd -V -l d_rt=3600 -l broadwell=true '${cmd1}'"
    if [ $? -eq 0 ]
    then
      echo "Successfully executed ilastik 1"
    else
    ...
    ssh login1 "source /etc/profile; export LAZYFLOW_THREADS=${LAZYFLOW_THREADS}; export LAZYFLOW_TOTAL_RAM_MB=${LAZYFLOW_TOTAL_RAM_MB}; qsub -sync y -pe batch 4 -N ml-${tile_name} -j y -o ${log_file_cluster_2} -b y -cwd -V -l d_rt=3600 -l broadwell=true '${cmd2}'"
    if [ $? -eq 0 ]
    then
      echo "Successfully executed ilastik 2"
      exit 0
