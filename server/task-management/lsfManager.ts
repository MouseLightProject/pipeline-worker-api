...
export class LSFTaskManager implements ITaskUpdateSource, ITaskManager {
    ...
    public startTask(taskExecution: ITaskExecution, taskDefinition: ITaskDefinition) {
        ...
        const requiredBsubArgs = ["-J", `ml-dg-${taskExecution.tile_id}`, "-cwd", `-R\\"select[broadwell]\\"`, "-g", `/mouselight/pipeline/${taskExecution.worker_id}`, "-oo", `${taskExecution.resolved_log_path + ".log"}`, "-eo", `${taskExecution.resolved_log_path + ".err"}`, `'${programArgs}'`];
